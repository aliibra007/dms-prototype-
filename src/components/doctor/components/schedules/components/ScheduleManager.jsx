      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-4 rounded-xl border-2" style={{ background: isDark ? COLORS.dark.cardBg : COLORS.light.cardBg, borderColor: isDark ? COLORS.dark.primary : COLORS.light.primary }}>
        <div className="flex items-center gap-4">
          ...
          {publishStatus.lastPublished && (
            <span className="text-sm" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>
        ...
        <button 
          ...
          style={{ background: publishStatus.status === 'published' ? COLORS.light.muted : '#10B981', cursor: publishStatus.status === 'published' ? 'default' : 'pointer' }}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {weeklySchedule.map((day, index) => (
              <div key={day.day} className="p-4 rounded-xl border-2 transition-all" 
                style={{ 
                  background: isDark ? COLORS.dark.cardBg : COLORS.light.cardBg, 
                  borderColor: day.enabled ? (isDark ? COLORS.dark.primary : COLORS.light.primary) : (isDark ? COLORS.dark.muted : COLORS.light.muted),
              ...
              >
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <div className="flex items-center gap-3 min-w-[120px]">
                    ...
                    />
                    <span className="font-bold" style={{ color: isDark ? COLORS.dark.text : COLORS.light.text }}>{day.day}</span>
                  </div>
                  ...
                  {day.enabled && (
                    <div className="flex flex-1 flex-wrap items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Interval:</span>
                        <select 
                          value={day.interval}
                      ...
                      <button 
                        ...
                        className="ml-auto text-xs flex items-center gap-1 px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                  ...
                  {!day.enabled && (
                    <span className="text-sm italic" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Day Off</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {activeSection === 'offdays' && (
        <div className="space-y-6">
          <div className="rounded-xl p-6 border-2" style={{ background: isDark ? COLORS.dark.cardBg : COLORS.light.cardBg, borderColor: isDark ? COLORS.dark.muted : COLORS.light.muted }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-end">
              ...
              <div className="flex-[2] space-y-2">
                <label className="text-sm font-medium" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Reason</label>
                <input 
                  type="text" 
          ...
          <div className="space-y-3">
            ...
            {offDays.length === 0 ? (
              <p style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>No off-days scheduled.</p>
            ) : (
              offDays.map((day) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>{day.reason}</p>
                    </div>
                  </div>
                  <button 
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Lunch Break</p>
                </div>
              </div>
              <button 
                className="p-2 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors"
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-lg" style={{ background: isDark ? COLORS.dark.secondary : COLORS.light.secondary }}>
                <div>
                  ...
                  <p className="text-sm" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Reserve specific slots for urgent appointments</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Slots per Day</label>
                  <input 
                    type="number" 
                    defaultValue={2}
                    className="w-full px-4 py-2 rounded-lg border outline-none"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium" style={{ color: isDark ? COLORS.dark.muted : COLORS.light.muted }}>Release Unused Slots (Hours Before)</label>
                  <input 
                    type="number" 
                    defaultValue={24}
                    className="w-full px-4 py-2 rounded-lg border outline-none"
